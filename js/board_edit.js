...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btn_file_dels.forEach((box) => {
    box.addEventListener("click", () => {
      ...
      f.append("th", box.dataset.th); // 파일 번호
  ...
  if (id_attach) {
    id_attach.addEventListener("change", () => {
      ...
      if (id_attach.files[0] > 40 * 1024 * 1024) {
        alert(
      ...
      if (
        ...
        alert(
          "첨부할 수 없는 포맷의 파일이 첨부되었습니다.(exe, txt, dmg, php, js ..)"
        );
        id_attach.value = "";
        return false;
      }
